    ...
    public class BookingController : ControllerBase
    {
        ...
        public async Task<IActionResult> AssignTechnician(int bookingId, int technicianId)
        {
            try
            {
                ...
                {
                    return StatusCode(403, new { message = "Only admin can assign technicians" });
                }
        ...
        public class DealerEmailRequest
        {
        ...
        public async Task<IActionResult> SendEmailDealer(int bookingId, [FromBody] DealerEmailRequest request)
        {
            try
            {
                var bookings = _context.Booking
                ...
                var bookingDTO = new BookingDTO
                {
                    ...
                    Note = bookings.Note,
                    Email = bookings.Customer.Email,
                    dealerEmail = request.EmailDealer,
                    ServiceName = "General Service" // Replace with actual service name if available
                };
        ...
        public async Task<IActionResult> SendEmailReminder(int bookingId)
        {
            try
            {
                ...
                BackgroundJob.Schedule(
                    ...
                    TimeSpan.FromMinutes(5) // Schedule to run after 5 minute for testing
                );
